...
export default () => {
  return {
    ...
    register: (ctx: FloatingLive) => {
      ...
      ctx.message.handler.register("bilibili.getavatar", (msg: Message.All) => {
            ...
            if (!user.avatar && msg.type == "chat") {
              if (avatarMap.has(user.id)) {
                ...
                queue.add(() => {
                  ...
                  axios
                    .get(
                      `https://api.bilibili.com/x/space/acc/info?mid=${user.id}`
                    )
                    .then((res) => {
                      if (res.data.code == 0) {
                        avatarMap.set(user!.id, res.data.data.face);
                        console.log(
                          `[${avatarMap.size}]从API获取: ${res.data.data.face}`
                        );
                      }
                    })
                    .catch((rej) => {

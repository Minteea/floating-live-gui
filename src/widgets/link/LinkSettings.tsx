      ...
      <div style={{display: "flex"}}>
        <Tooltip title={store.link.link_connected ? "重新连接" : "连接"}>
          <Button
            type="primary"
            shape="circle"
            icon={store.link.link_connected ? <ReloadOutlined /> : <LinkOutlined />}
            onClick={() => {
              runInAction(() => {store.link.link = inputLink})
        ...
        <Input value={inputLink}
          ...
          placeholder={"连接后端url"}
          status={(store.link.link == inputLink || !store.link.link_connected) ? "" : "warning"}
        />

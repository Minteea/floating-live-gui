...
const ServerSettings: React.FC = function () {
  return (
    <div>
      <div>
        启用websocket服务
        <Switch checked={store.server.send_message}
          onClick={() => {controller.server.sendMessage(!store.server.send_message)}}
        />
      </div>
      <div>端口号
        <InputNumber
          ...
          onBlur={(e) => {
            runInAction(() => { controller.server.port(Number(e.target.value)) })
          }}
        />
      </div>
    </div>

    ...
    <div>
      <div>
        ...
        <Switch
          ...
          onClick={() => {controller.saving.saveMessage(!store.saving.save_message);}}
        />
      </div>
      <div>
        记录服务器弹幕源数据
        <Switch checked={store.saving.save_origin}
          onClick={() => { controller.saving.saveOrigin(!store.saving.save_origin);}}
        />
      </div>
      <div>
        保存路径
        <div style={{
          ...
          <Input value={inputSavePath}
            onChange={(e) => {
              changeInputSavePath(e.target.value)
            }}
            onBlur={(e) => {
              if (!store.living.started && !(store.saving.save_path == inputSavePath)) {
                controller.saving.savePath(e.target.value)
              }
            }}
            status={(store.saving.save_path == inputSavePath || !store.living.started) ? "" : "warning"}
          />

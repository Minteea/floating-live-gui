...
const PageAbout: React.FC = function() {
  return (
    <div>
      <h2>关于</h2>
      <div>
        <div><b>Floating Living GUI</b> {version.app ? `[DEV] 版本 v${version.app}` : null}</div>
        ...
        <div>----------</div>
        {
          version.client == "electron" ? <>
            ...
            <div>操作系统平台: {version.platform?.[0]} {version.platform?.[1]}</div>
          </> : <>
            <div>浏览器版本: {version.browser?.[0]} {version.browser?.[1]} ({version.browser?.[2]})</div>
          </>
        }
      </div>
      <div>
